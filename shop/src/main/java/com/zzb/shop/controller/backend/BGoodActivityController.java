 ...
 */
@Controller
@RequestMapping(value = "/backend/goodActivity")
public class BGoodActivityController extends BaseController{
	 ...
	 * @return
	 */
	@RequestMapping(value = "goodActivity")
	public String sysUser(Model model) {
		return "backend/goodActivity/goodActivity";
	}
	/**
	 * 列表数据页
	 * @param model
	 ...
	 * @return
	 */
	@RequestMapping(value = "/goodActivityList")
	@ResponseBody
	public Object sysUserList(Model model,Page page) {
		...
		page.setPd(pd);
		List<PageData> list = goodActivityService.list(page);// 列出用户列表
		int total = goodActivityService.listCount(page);
	...
	@RequestMapping(value = "/delGoodActivity")
	@ResponseBody
	public Object delSysUser(Page page){
		...
		try {
			...
			for (int i = 0, l = ids.length; i < l; i++) {
				String id = ids[i];
				goodActivityService.deleteByPrimaryKey(Long.parseLong(id));
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	 ...
	 * @return
	 */
	@RequestMapping(value = "editGoodActivity")
	public String editSysUser(Model model) {
		return "backend/goodActivity/editGoodActivity";
	}
	/**
	 * 获取用户
	 * @param page
	 * @return
	 */
	@RequestMapping(value = "/getGoodActivity")
	@ResponseBody
	public Object getSysUser(Page page){
		...
		try{
			...
			GoodActivity domain = goodActivityService.selectByPrimaryKey(Long.parseLong(id));
	 ...
	 * @return
	 */
	@RequestMapping(value = "/saveGoodActivity")
	@ResponseBody
	public Object saveSysUser(GoodActivity domain){
		...
		try {
			...
			if (StringUtil.isNullOrEmpty(domain.getId())){
				...
				//domain.setDelFlag("0");
				goodActivityService.insert(domain);
			}else{
				
				//domain.setUpdateDate(date);
				goodActivityService.updateByPrimaryKeySelective(domain);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			pushMsg.setStatus(false);
