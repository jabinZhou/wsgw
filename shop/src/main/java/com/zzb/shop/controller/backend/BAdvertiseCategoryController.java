 ...
 */
@Controller
@RequestMapping(value = "/backend/advertiseCategory")
public class BAdvertiseCategoryController extends BaseController{
	 ...
	 * @return
	 */
	@RequestMapping(value = "advertiseCategory")
	 ...
	 * @return
	 */
	@RequestMapping(value = "/advertiseCategoryList")
	@ResponseBody
	...
	public Object delSysUser(Page page){
		...
		try {
			...
			for (int i = 0, l = ids.length; i < l; i++) {
				String id = ids[i];
				advertiseCategoryService.deleteByPrimaryKey(Long.parseLong(id));
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	 ...
	 * @return
	 */
	@RequestMapping(value = "editAdvertiseCategory")
	public String editSysUser(Model model) {
		return "backend/advertiseCategory/editAdvertiseCategory";
	}
	/**
	 * 获取用户
	 * @param page
	 * @return
	 */
	@RequestMapping(value = "/getAdvertiseCategory")
	@ResponseBody
	public Object getSysUser(Page page){
		...
		try{
			...
			AdvertiseCategory domain = advertiseCategoryService.selectByPrimaryKey(Long.parseLong(id));
	 ...
	 * @return
	 */
	@RequestMapping(value = "/saveAdvertiseCategory")
	@ResponseBody
	public Object saveSysUser(AdvertiseCategory domain){
		...
		try {
			...
			if (StringUtil.isNullOrEmpty(domain.getId())){
				...
				domain.setDelFlag("0");
				advertiseCategoryService.insert(domain);
			}else{
				
				domain.setUpdateDate(date);
				advertiseCategoryService.updateByPrimaryKeySelective(domain);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			pushMsg.setStatus(false);
