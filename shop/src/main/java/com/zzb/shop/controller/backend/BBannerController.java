...
public class BBannerController extends BaseController{
	...
	public Object sysUserList(Model model,Page page) {
		...
		page.setPd(pd);
		List<PageData> list = bannerService.list(page);// 列出用户列表
		int total = bannerService.listCount(page);
	...
	public Object delSysUser(Page page){
		...
		try {
			...
			for (int i = 0, l = ids.length; i < l; i++) {
				String id = ids[i];
				bannerService.deleteByPrimaryKey(Long.parseLong(id));
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public String editSysUser(Model model) {
		return "backend/banner/editBanner";
	}
	/**
	 * 获取用户
	 * @param page
	...
	public Object getSysUser(Page page){
		...
		try{
			...
			Banner domain = bannerService.selectByPrimaryKey(Long.parseLong(id));
	...
	@ResponseBody
	public Object saveSysUser(Banner domain){
		...
		try {
			...
			if (StringUtil.isNullOrEmpty(domain.getId())){
				...
				domain.setDelFlag("0");
				bannerService.insert(domain);
			}else{
				
				domain.setUpdateDate(date);
				bannerService.updateByPrimaryKeySelective(domain);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			pushMsg.setStatus(false);
