 ...
 */
@Controller
@RequestMapping(value = "/backend/goodDetail")
public class BGoodDetailController extends BaseController{
	...
	public String sysUser(Model model) {
		return "backend/goodDetail/goodDetail";
	}
	/**
	 * 列表数据页
	 * @param model
	 ...
	 * @return
	 */
	@RequestMapping(value = "/goodDetailList")
	@ResponseBody
	public Object sysUserList(Model model,Page page) {
		...
		page.setPd(pd);
		List<PageData> list = goodDetailService.list(page);// 列出用户列表
		int total = goodDetailService.listCount(page);
	...
	@RequestMapping(value = "/delGoodDetail")
	@ResponseBody
	public Object delSysUser(Page page){
		...
		try {
			...
			for (int i = 0, l = ids.length; i < l; i++) {
				String id = ids[i];
				goodDetailService.deleteByPrimaryKey(Long.parseLong(id));
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	 ...
	 * @return
	 */
	@RequestMapping(value = "editGoodDetail")
	public String editSysUser(Model model) {
		return "backend/goodDetail/editGoodDetail";
	}
	/**
	 * 获取用户
	 * @param page
	...
	public Object getSysUser(Page page){
		...
		try{
			...
			GoodDetail domain = goodDetailService.selectByPrimaryKey(Long.parseLong(id));
	 ...
	 * @return
	 */
	@RequestMapping(value = "/saveGoodDetail")
	@ResponseBody
	public Object saveSysUser(GoodDetail domain){
		...
		try {
			...
			if (StringUtil.isNullOrEmpty(domain.getId())){
				...
				//domain.setDelFlag("0");
				goodDetailService.insert(domain);
			}else{
				
				//domain.setUpdateDate(date);
				goodDetailService.updateByPrimaryKeySelective(domain);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			pushMsg.setStatus(false);
