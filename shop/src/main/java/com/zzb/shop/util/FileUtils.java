...
public class FileUtils {
    ...
    public static Boolean fileTypeVaild(String type,String fileName){
        ...
        if (!Arrays.<String> asList(extMap.get(type).split(",")).contains(FilenameUtils.getExtension(fileName.toLowerCase()))) {// 检查扩展名
            return false;
        }
        return true;
    }
    ...
    public static String imgScale(String dirName,MultipartFile file,int height, int width, boolean flag){
        ...
        try {
            ...
            if(height==0){
            	height=bi.getWidth()/width*bi.getHeight();
            }
    ...
    public static String upload(String type,String dirName,MultipartFile file){
    	 ...
    	 try{
    		 ResourceBundle upload=ResourceBundle.getBundle("upload");
    		 String key=type+"/"+dirName+"/"+UUID.randomUUID()+"."+FilenameUtils.getExtension(file.getOriginalFilename());
    		 File dirFile = new File(upload.getString("rootPath")+key);
	 	     if (!dirFile.exists()) {
	 	        dirFile.mkdirs();
	 	     }
	...
	public static String download(HttpServletRequest request,
		...
		try {
			...
			response.setHeader("Content-Disposition", "attachment; filename="+ new String(file.getName().getBytes("UTF-8"),"ISO-8859-1"));//fileName
			// 下载框的信息
